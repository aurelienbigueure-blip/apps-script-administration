...
/**
 * Menu unique exposé dans Google Sheets.
 */
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu('Compta Admin')
    .addItem('1. Lancer traitement Gmail', 'runComptaMailProcessing')
    .addItem('2. Analyser 5 actions ouvertes avec IA', 'analyserActionsOuvertesRapideIA')
    .addItem('3. Appliquer décisions IA évidentes', 'appliquerDecisionsIAEvidentes')
    .addItem('4. Mettre à jour Domaines Suivi', 'majDomainesSuiviDepuisActions')
    .addSeparator()
    .addItem('5. Audit colonnes', 'auditColonnesComptaAdmin')
    .addItem('6. Mise en forme rapide', 'setupMiseEnFormeRapideComptaAdmin')
    .addItem('7. Réparer interface simple', 'reparerInterfaceComptaAdmin')
    .addToUi();
}
function testTypeProjet() {
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  Logger.log(ss ? ss.getName() : 'Aucun spreadsheet actif : projet probablement autonome');
}
